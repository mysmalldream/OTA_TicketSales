        ...
        <div class="form-box">
            <el-form ref="form" :model="form" label-width="100px" enctype="multipart/form-data" method="post">
                ...
                <el-form-item label="开始/结束日期" prop="time">
                    <div class="block">
                        <el-date-picker v-model="value3" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change='dates'>
                        </el-date-picker>
                    </div>
                </el-form-item>
                ...
                <el-form-item label="上传照片" prop="file">
                    <el-upload class="upload-demo" ref="upload" :action=urls :on-preview="handlePictureCardPreview" :on-remove="handleRemove" :data=lists 
                         list-type="picture-card" :file-list="fileList" :auto-upload="true" accept=".jpg,.png,.jpeg,.gif,.bmp">
    ...
    export default {
        data: function () {
            return {
                ...
                urls: common.apidomain + "/hotProduct/addImages.action", //上传图片文件地址
                fileList: [],
                currentPage: 1, //当前页码数
                dragData: { //地图
                    lng: null,
        ...
        methods: {
            ...
            onSubmit(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post(common.apidomain + "/hotProduct/add.action?name=" + this.form.name + "&viewName=" + this.form.viewName + "&salePrice=" + this.form.salePrice + "&marketPrice=" + this.form.marketPrice + "&num=" + this.form.num + "&notice=" + this.form.notice + "&costInside=" + this.form.costInside + "&costOutside=" + this.form.costOutside + "&remark=" + this.form.remark + "&introduce=" + this.form.introduce + "&method=" + this.form.method + "&startTime=" + this.startTime + "&endTime=" + this.endTime+ "&random_no=" + window.sessionStorage.getItem("random_no")).then((res) => {
                            console.log(res.data)
                            ...
                            if (this.codesID === 0) {    //参数错误
                                ...
                                this.$router.push({ path: './Sparpreis', params: { currentPage: this.currentPage } });
                            }
                        })
                    } else {
                        this.$message({
