...
(function ($) {
      ...
      for (var k = 1; k <= 7; k++){
        ...
        if (d > 0 && d <= thisMonthDays){
          ...
          if (tdId >= startDay && tdId <= endDay) {
            html += '<td class="valid-hook" data-week="' + (k - 1) + '" data-id="' + tdId + '"><b>' + d + '</b><div class="data-hook" style="color:red;"></div></td>';
          } else {
  ...
  fn.initSettingWindow = function () {
    ...
    html += '       <a class="cddsw-close guanbi"><i></i></a>';
    html += '   </div>';
    ...
    html += '       <legend class="bs-title"><b style="color:red;">如需批量设置,请手动输入日期范围勾选启用,并点击启用本设置>>>启用</b></legend>';
    ...
    html += '       <button class="btn-confirm qiyong">启用本设置</button>';
    html += '       <button class="btn-cancel quxiao">取消</button>';
    html += '   </div>';
    html += '</div>';
  ...
  fn.handleClickEvent = function () {
    ...
    this.calendar.on('click', '.btn-confirm', function () {
     ...
     $(".qiyong").on("click", function() {
         //  alert(43252)
         $(".capricorncd-date-detailed-settings").hide(); //点击启用修改,弹框隐藏(自定义修改部分)
     });
    // 取消
    ...
    $('.quxiao').on('click',function () {  
      //  alert(43252)
      $(".capricorncd-date-detailed-settings").hide();    //点击取消修改,弹框隐藏(自定义修改部分)
    })
    // 获取点击日期数据
    // 渲染设置框内容
    // 显示设置窗口
    ...
    $(".guanbi").on("click", function() {
        //  alert(43252)
        $(".capricorncd-date-detailed-settings").hide(); //点击X关闭修改,弹框隐藏(自定义修改部分)
    });
    // 保存设置
